...
class StorageAPI implements Storage {
  ...
  async createTask(content: string, params: {date?: string; time?: string; timezone?: string}): Promise<Day> {
    ...
    const maxOrder = Math.max(...allTasks.filter((t) => t.scheduled.date === scheduledDate).map((t) => t.scheduled.order ?? 0), 0)
    ...
    const dayId = scheduledDate.replaceAll("-", "")
  ...
  /**
   * Move task to a new position within the same day
   * @param taskId - The id of the task to move
   * @param beforeTaskId - The id of the task to move before
   * @returns The day that matches the query
   */
  async moveTask(taskId: Task["id"], beforeTaskId: Task["id"] | null): Promise<Day | null> {
    const allTasks = await window.electronAPI.loadTasks()
    ...
    const tasksForDay = allTasks
      .filter((t) => t.scheduled.date === date && t.id !== taskId)
      .sort((a, b) => (a.scheduled.order ?? 0) - (b.scheduled.order ?? 0))
    ...
    if (beforeTaskId === null) {
      ...
      newOrder = (first?.scheduled.order ?? 1000) / 2
    } else {
      const idx = tasksForDay.findIndex((t) => t.id === beforeTaskId)

      if (idx === -1) {
        const last = tasksForDay[tasksForDay.length - 1]
        newOrder = (last?.scheduled.order ?? 0) + 1000
      } else {
        const prev = tasksForDay[idx - 1]
        const next = tasksForDay[idx]

        const prevOrder = prev?.scheduled.order ?? 0
        const nextOrder = next?.scheduled.order ?? prevOrder + 2000

        if (Math.abs(nextOrder - prevOrder) < 1e-3) {
          setTimeout(() => this.normalizeOrdersForDate(date), 0)
        }
    ...
    await window.electronAPI.saveTasks(updatedTasks)
  ...
  /**
   * Normalize the orders for a given date
   * @param date - The date formatted as YYYY-MM-DD
   */
  private async normalizeOrdersForDate(date: ISODate) {
    const allTasks = await window.electronAPI.loadTasks()
    const tasksForDate = allTasks.filter((t) => t.scheduled.date === date).sort((a, b) => (a.scheduled.order ?? 0) - (b.scheduled.order ?? 0))

    tasksForDate.forEach((t, idx) => {
      t.scheduled.order = (idx + 1) * 1000
    })
