...
type Step = "address" | "confirm" | "claim" | "waiting" | "denied" | "expired"
...
const title = computed(() => STEP_TITLES[step.value])
...
function onCodeDigitInput(index: number, event: Event): void {
  ...
  if (digit && index < 5) focusCodeCell(index + 1)
}
...
function onCodeDigitPaste(index: number, event: ClipboardEvent): void {
  ...
  focusCodeCell(Math.min(index + digits.length, 5))
}
...
<template>
  <BaseModal :title="title" container-class="mx-4 h-auto w-full max-w-md rounded-xl" content-class="!p-5" @close="$emit('close')">
    ...
    <div v-else-if="step === 'claim'" class="flex flex-col gap-4">
      ...
      <div v-focus-on-mount class="flex items-center justify-center gap-2" role="group" aria-label="Six-digit claim code">
      ...
      <BaseButton variant="primary" size="sm" class="w-full py-1.5" :disabled="!code.trim()" :loading="isLoading" @click="onClaim"
        >Connect</BaseButton
      >
    </div>
