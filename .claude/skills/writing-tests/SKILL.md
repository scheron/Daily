...
| Layout: scroll position, overflow, sizes | none — happy-dom lays nothing out. The brief names a gate B driver over CDP; an assertion on a proxy (a height, a class) says it is one in the test's name | the collapsed-column test in `TaskBoard.test.ts`                               |
