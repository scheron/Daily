 ...
 * Server (`tests/helpers/syncServer.ts`) and a real temp directory standing in for iCloud. Nothing
...
import {ProviderMigrationService} from "@main/storage/sync/ProviderMigrationService"
...
import {buildSyncRemotes} from "@main/utils/sync/syncProvider"
...
/**
 * `ICloudRemoteAdapter` reads/writes through `@/utils/fileCoordinator`, whose real implementation
 * assumes a genuine iCloud-managed path. Standing a plain temp directory in for iCloud (the
 * technique `tests/main/storage/sync/integration.test.ts` already uses) needs this same mock.
 */
