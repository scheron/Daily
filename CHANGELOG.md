...
- **Updates** — an update that failed partway could leave the app unable to start. The new version is now checked before it replaces the old one, and if anything goes wrong the previous version is put back and opened instead.
- **Daily Sync Server** — `daily.sh upgrade` now carries a server across a protocol change. It used to re-pull the version it already had, so a server could never catch up with an app that had moved ahead. It also takes a backup first and restores it if the upgrade fails.
...
- `SyncEngine` fully rewritten: push-delta → pull-delta → field-merge → apply
