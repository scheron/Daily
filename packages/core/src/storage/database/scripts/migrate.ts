...
export type MigrationStep = string | ((db: SqliteDriver) => void)
...
export function runMigrations(db: SqliteDriver) {
...
export function rollbackLastMigration(db: SqliteDriver): number | null {
...
export function getAppliedMigrations(db: SqliteDriver): MigrationRecord[] {
  return db.prepare("SELECT version, name, applied_at FROM _migrations ORDER BY version ASC").all() as MigrationRecord[]
}

function runStep(db: SqliteDriver, step: MigrationStep) {
