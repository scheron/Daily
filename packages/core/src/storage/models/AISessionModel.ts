...
/** Lifecycle states an AgentTurn is persisted with. */
export type AgentTurnStatus = "running" | "completed" | "failed" | "cancelled" | "waiting_confirmation"

/**
 * One observable event within a turn, as it is stored. The payload of each
 * variant is written and read back as opaque JSON here — the agent-loop types
 * that describe it live in the app that runs the loop.
 */
export type AgentStep =
  | {id: string; type: "model_response"; createdAt: number; message: any; reasoning?: string; reasoningDurationMs?: number}
  | {id: string; type: "tool_call"; createdAt: number; toolCallId: string; toolName: string; params: unknown}
  | {id: string; type: "tool_result"; createdAt: number; toolCallId: string; toolName: string; result: any}
  | {id: string; type: "respond"; createdAt: number; text: string; reasoning?: string; reasoningDurationMs?: number}
  | {id: string; type: "confirmation_request"; createdAt: number; confirmation: any}
  | {id: string; type: "error"; createdAt: number; message: string}

/** One user request and everything the assistant did for it, as the store of record holds it. */
