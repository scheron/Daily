...
import ChatMarkdown from "./{fragments}/ChatMarkdown"
import MessageReasoning from "./{fragments}/MessageReasoning.vue"
import ToolCallGroup from "./{fragments}/ToolCallGroup"
...
function getRootClasses(isUser: boolean) {
  return cn("flex", isUser ? "justify-end" : "justify-start")
}
</script>

<template>
  <div :class="getRootClasses(isUser)">
      ...
      <div class="min-w-0 flex-1">
        <div class="ai-message-content flex flex-col gap-2 text-sm select-text">
          ...
          <template v-else-if="message.status === 'failed'">
            ...
            <BaseButton v-if="canRetryFailed" variant="ghost" icon="refresh" class="self-start" @click="aiStore.retryMessage"> Retry </BaseButton>
          </template>
        </div>
