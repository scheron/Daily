...
const codeCellElsRef = useTemplateRef<HTMLInputElement[]>("codeCellEls")
...
function onOpenServerDocs() {
  window.BridgeIPC["shell:open-external"]("https://github.com/scheron/Daily/blob/main/apps/server/README.md")
}
...
function startPolling() {
  stopPolling()
  pollTimer = setInterval(pollTick, 2_000)
}
...
function focusCodeCell(index: number): void {
  codeCellElsRef.value?.[index]?.focus()
}
