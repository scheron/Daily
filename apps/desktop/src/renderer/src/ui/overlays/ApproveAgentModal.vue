...
<template>
  <BaseModal title="Approve agent" container-class="mx-4 h-auto w-full max-w-md rounded-xl" content-class="!p-5" @close="$emit('close')">
    ...
    <div class="flex justify-between gap-10">
      <div>
        <div class="text-base-content/50 text-xs tracking-[0.06em] uppercase">Agent</div>
        <div class="text-base-content mt-1 flex items-center gap-1.5 text-sm font-medium">
          <BaseIcon :name="toAgentIconName(request.agentName)" class="size-4" />
          {{ request.agentName }}
        </div>
      </div>
      <div class="text-right">
        <div class="text-base-content/50 text-xs tracking-[0.06em] uppercase">Returns to</div>
    ...
    <div class="mt-3.5">
      <div class="text-base-content/50 text-xs tracking-[0.06em] uppercase">Belongs to</div>
      <div class="text-base-content mt-1 flex items-center gap-1.5 text-sm font-medium">
        {{ deviceName }}
