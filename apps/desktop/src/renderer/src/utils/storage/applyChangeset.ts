 ...
 * another window's broadcast, or a sync pull. A collection left out of `target` is not touched.
 * Rows are reconciled by value and never by `updatedAt`, so a row whose fields already match
 * stays the same object and a second application of the same changeset changes nothing. A row
 * upserted with `deletedAt` set leaves its collection, which holds live rows only.
 *
