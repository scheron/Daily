...
contextBridge.exposeInMainWorld("BridgeIPC", {
  ...
  "tasks:get-backlog": (params?: {limit?: number; branchId?: Branch["id"]}) => ipcRenderer.invoke("tasks:get-backlog", params) as Promise<Task[]>,
  "tasks:get-one": (id: Task["id"]) => ipcRenderer.invoke("tasks:get-one", id) as Promise<Task | null>,
  "tasks:update": (id: Task["id"], updates: PartialDeep<Task>, activeDay?: ISODate) => ipcRenderer.invoke("tasks:update", id, updates, activeDay),
  ...
  "tasks:schedule": (id: Task["id"], schedule: TaskSchedule) => ipcRenderer.invoke("tasks:schedule", id, schedule) as Promise<Task | null>,
  "tasks:move-to-backlog": (id: Task["id"]) => ipcRenderer.invoke("tasks:move-to-backlog", id) as Promise<Task | null>,
  ...
  "tasks:move-in-trash": (taskId: Task["id"], targetTaskId: Task["id"] | null, position: TaskMovePosition) =>
    ipcRenderer.invoke("tasks:move-in-trash", taskId, targetTaskId, position) as Promise<Task | null>,
  "tasks:restore": (id: Task["id"], activeDay?: ISODate) => ipcRenderer.invoke("tasks:restore", id, activeDay) as Promise<Task | null>,
