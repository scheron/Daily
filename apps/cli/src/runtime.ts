import {createStorageCore, FolderRemoteAdapter, initDatabase, SyncEngine} from "@daily/core"

import {CliController} from "./CliController"
import {resolveCliRuntime} from "./config"
import {createNodeSqliteDriver} from "./database/nodeSqliteDriver"
import {exitCodeFor, renderJsonError} from "./output"

import type {StorageCore} from "@daily/core"
...
export async function runCliCoreCommand(opts: {json?: boolean}, run: (core: StorageCore, runtime: CliRuntime) => Promise<void>): Promise<void> {
  try {
    ...
    const db = initDatabase(createNodeSqliteDriver(runtime.paths.dbPath()))
