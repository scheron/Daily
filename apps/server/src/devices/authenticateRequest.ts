...
import {readBearerToken} from "../http/readBearerToken"
...
export function authenticateRequest(store: ServerStore, req: IncomingMessage): DeviceRecord {
  ...
  if (token === null) throw new ProtocolError(ProtocolErrorCode.UNAUTHORIZED)

  const device = findDeviceByTokenHash(store, hashToken(token))
