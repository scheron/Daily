...
class Logger {
  ...
  private log(level: LogLevel, context: LogContext | undefined, message: string, data?: any): void {
    ...
    const parts: string[] = [`${dim}${timestamp}${reset}`, `${color}${level.padEnd(5)}${reset}`]
...
export const logger = new Logger(Env.logging.enabled(), Env.logging.minLevel, Env.logging.contexts)
